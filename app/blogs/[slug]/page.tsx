...
export default function BlogDetailPage({ params }: BlogDetailPageProps) {
  ...
  const relatedBlogs = blogs.filter((b) => b.slug !== params.slug).slice(0, 3)
    ...
    <div className="min-h-screen bg-gradient-to-b from-background to-muted/20">
      ...
      <article className="container mx-auto px-4 py-12">
        <div className="max-w-4xl mx-auto">
          ...
          {/* Blog Content */}
          <div className="bg-white/80 backdrop-blur-sm rounded-2xl p-8 md:p-12 border border-border/50 shadow-sm mb-12">
          ...
          <section>
            ...
            <div className="grid md:grid-cols-3 gap-8">
              {relatedBlogs.map((relatedBlog) => (
                <article
                  key={relatedBlog.id}
                  className="bg-white/80 backdrop-blur-sm rounded-xl overflow-hidden border border-border/50 shadow-sm hover:shadow-lg transition-all duration-300 product-card-hover"
                >
